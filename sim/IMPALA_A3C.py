...
def compute_gradients(s_batch, a_batch, r_batch, terminal, actor, critic):
    ...
    # IMPALA: Calculate delta_tV
    # IMPALA: Find action taken at each step (argmax axis=1 for a_batch)
    target_actions = np.argmax(a_batch, axis=1)
    target_action_probs = np.max(a_batch, axis=1)
    
    # IMPALA: Predict action probabilities using behaviour model, take probs corresponding to previous argmax
    behaviour_actions = actor.predict(np.reshape(s_batch, (-1, S_INFO, S_LEN)))
    behaviour_action_probs = np.zeros(target_action_probs.shape)
    for idx, best_action in enumerate(target_actions):
        behaviour_action_probs[idx] = behaviour_actions[idx, best_action]
        
    # IMPALA: Divide and threshold to calculate rho (perform op in log space?)
    rhos = np.minimum(RHO_THRESHOLD, behaviour_action_probs / target_action_probs)
    # IMPALA: Shift left v_batch by 1 (np.roll with axis=1), discount, add r_batch and subtract v_batch
    # IMPALA: Multiply by rho (exp(rho) if taken log)
    v_batch_plus1 = np.roll(v_batch, -1)
    v_batch_plus1[-1] = 0
    discounted_v_batch = GAMMA * v_batch_plus1
            
    delta_tV_batch = rhos.reshape((-1, 1)) * (r_batch + discounted_v_batch - v_batch)
    
    # IMPALA: Calculate c
    # IMPALA: c is cumulative product from reverse of rho
    cs = np.copy(rhos)
    # for i in range(cs.shape[0] - 2, -1, -1):
    #     cs[i] *= cs[i + 1]
    cs = np.minimum(cs, C_THRESHOLD)
    
    # IMPALA: Initialize bootstrap. (How? Take v - V = 0 for last state?)
    v_trace_batch = v_batch + delta_tV_batch
    # IMPALA: Start from end and calculate vs = Vs + delta_tVs + gamma * cs * (vs+1 - Vs+1)
    for i in range(v_trace_batch.shape[0] - 2, -1, -1):
        v_trace_batch[i] += GAMMA * cs[i] * (v_trace_batch[i + 1] - v_batch[i + 1])
    # IMPALA: Replace V everywhere with v
    ...
    if terminal:
        ...
        R_batch[-1, 0] = v_trace_batch[-1, 0]  # boot strap from last state

    for t in reversed(xrange(ba_size - 1)):
        R_batch[t, 0] = r_batch[t] + GAMMA * R_batch[t + 1, 0]

    td_batch = R_batch - v_trace_batch
